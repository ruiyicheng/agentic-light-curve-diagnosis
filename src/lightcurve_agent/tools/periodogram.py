...
def obtain_GLS(
    ...
    try:
        ...
        # Compute periodogram
        periodogram = compute_gls_periodogram(lc)

        best_period = periodogram.best_value
        best_power = periodogram.best_power

        print(f"Best period: {best_period:.5f} days with power {best_power:.5f}")
        ...
        for label, (period_val, filename) in results.items():
            t_folded = phase_fold(lc, period_val)
            folded_lc = load_light_curve.__wrapped__(None)  # placeholder to get type

            # Create temporary LC with folded data
            import pandas as pd
...
def obtain_BLS(
    ...
    try:
        ...
        periodogram = compute_bls_periodogram(lc, duration=settings.bls_default_duration)

        best_period = periodogram.best_value
        best_power = periodogram.best_power

        print(f"Best BLS period: {best_period:.5f} days with power {best_power:.5f}")
        ...
        for label, (period_val, filename) in results.items():
            ...
            t_folded = pd.Series((np.asarray(lc.time) / period_val) % 1.0)
